...
for ii=1:num_conditions
    ...
    dwell_skewness = get_empirical_dwell_skewness(traces,num_states,dt);
    ...
    transition_cv = get_transition_path_cv(traces,num_states,dt);

    % create data structure
    temp_struct = struct();
    temp_struct.dwell_means = dwell_means; % dwell time mean, mu
    temp_struct.dwell_variance = dwell_variance; % dwell time variance, sigma^2
    temp_struct.dwell_skewness = dwell_skewness;
    ...
    temp_struct.transition_path_cv = transition_cv;
    
    dwell_std = sqrt(dwell_variance);
    
    temp_struct.dwell_cv = dwell_std ./ dwell_means; % dwell time CV (sigma/mu)
