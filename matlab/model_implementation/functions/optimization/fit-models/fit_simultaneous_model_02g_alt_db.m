function [x, fx] = fit_simultaneous_model_02g_alt_db(seed, substrate, alpha, balance_function)
...
% load the model
base_model = load_model('../Models/cas-simultaneous-model-02g-alt-db.json');
num_substrates = base_model.num_substrates;
num_variants = base_model.num_variants;

% generate weights for fitting data points
[on_sub_model, on_data_vector, weight_vector] = create_on_target_sub_model(balance_function);
[off_sub_model, off_data_vector] = create_off_target_sub_model(balance_function);
...
model_func = @(x)simultaneous_model(x,base_model,on_sub_model,off_sub_model,balance_function);
...
output_folder = "../Outputs/Model-02G/Alt-DB-02/sub_"+num2str(substrate)+"/"+"alpha_"+num2str(alpha)+"/";
...
%% Local Functions
function [sub_model_func, data_vector, weight_vector] = create_on_target_sub_model(balance_function)
% optimization settings
...
sub_model_func = @(pset)sub_model(pset,balance_function,fitting_options);
data_vector = @(dataset)create_data_vector(dataset,fitting_options);
weight_vector = generate_weight_vector(fitting_options);
end

function [sub_model_func, data_vector] = create_off_target_sub_model(balance_function)
% optimization settings
