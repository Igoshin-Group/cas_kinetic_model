function [x, fx] = fit_simultaneous_model_02g_tp(seed, substrate, alpha)
...
% load the model
base_model = load_model('../models/simultaneous/cas-simultaneous-model-02g.json');
num_substrates = base_model.num_substrates;
num_variants = base_model.num_variants;

% generate weights for fitting data points
[on_sub_model, on_data_vector, weight_vector_on] = create_on_target_sub_model();
[off_sub_model, off_data_vector, weight_vector_off] = create_off_target_sub_model();
weight_vector = [repmat(weight_vector_on,num_variants,1); ...
                 repmat(weight_vector_off,num_variants,1)]; % duplicate for each substrate
...
output_folder = "../output/fits/simultaneous/model-02g/transition_paths/sub_"+num2str(substrate)+"/"+"alpha_"+num2str(alpha)+"/";
...
fitting_options.transition_path_cv = true;
