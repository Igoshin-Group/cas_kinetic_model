function score = on_target_model_fit(p, data, weights, model, alpha)
...
for ii = 1:length(mfpt)
    if mfpt_obs(ii) > 6e4 % valid range of Jones NucleaSeq experiment
        mfpt_penalty = (heaviside(mfpt_obs(ii) - mfpt(ii)) * quadratic_penalty(mfpt_obs(ii), mfpt(ii)));
