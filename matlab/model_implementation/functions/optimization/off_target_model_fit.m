function score = off_target_model_fit(p, data, weights, model, alpha)
...
% % add qualitative penalty for binding fluxes
% for ii = 1:length(psets)
%     score = score + qualitative_penalty(psets(ii)); % add penalty for violating qualitative constraint
% end

% add penalty for cleavage MFPT
detection_threshold = 6e4;
for ii = 1:length(mfpt)
    if mfpt_obs(ii) > detection_threshold % valid range of Jones NucleaSeq experiment
        mfpt_penalty = (heaviside(detection_threshold - mfpt(ii)) * quadratic_penalty(mfpt_obs(ii), mfpt(ii)));
        score = score + mfpt_penalty;
    else
        mfpt_penalty = quadratic_penalty(mfpt_obs(ii), mfpt(ii));
        score = score + mfpt_penalty;
    end
    % fprintf("MFPT-Obs: %4.2f | MFPT-Pred: %4.2f | MFPT-Penalty: %4.2f\n", mfpt_obs(ii), mfpt(ii), mfpt_penalty);
end
end
...
function penalty = qualitative_penalty(p)
% add qualitative penalty for initial fluxes J1 > J2
...
penalty = g1 .* heaviside(J2(p) - J1(p)) .* quadratic_penalty(J1(p), J2(p)) + ...
          g2 .* heaviside(J2(p) - J1(p));
end