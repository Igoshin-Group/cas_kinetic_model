function [model, energies] = generate_off_target_model_from_energies(x, energies, C)
    % re-shape x to extract the parameters for each variant
    x = reshape(x, [], 2); % should have shape (16,2)

    % compute model parameter sets
    [off_model_wt, energies_wt] = helper_function(x(:,1), energies(1), C);
    [off_model_hf1, energies_hf1] = helper_function(x(:,2), energies(2), C);
...
function [model, energies] = helper_function(x, energies, C)
