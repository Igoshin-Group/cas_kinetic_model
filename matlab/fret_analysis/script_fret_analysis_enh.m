...
%% Load the Enh-Cas9 FRET data
enh_fret_file_paths = ["../../data/fret/processed/enh/enh-0bp_traces.dat",...
                      "../../data/fret/processed/enh/enh-1bp_traces.dat",...
                      "../../data/fret/processed/enh/enh-2bp_traces.dat",...
                      "../../data/fret/processed/enh/enh-3bp_traces.dat",...
                      "../../data/fret/processed/enh/enh-4bp_traces.dat"];
% load FRET datasets
...
title("Enh-Cas9 FRET Trace Length Distribution","FontSize",18);
...
title("Enh-Cas9 Transition Probability Matrix","FontSize",18);
...
dwell_times = cell(length(enh_fret_data),num_states);
num_conditions = length(enh_fret_data);
for ii = 1:num_conditions
    cur_condition = enh_fret_data(ii);
    num_traces = length(cur_condition.traces);
...
for ii = 1:num_conditions
    ...
    for jj = 1:num_states
        ...
        means_ci(ii,jj,:) = [mean_ij, mean_ij];
        vars_ci(ii,jj,:) = [var_ij, var_ij];
        skews_ci(ii,jj,:) = [skew_ij, skew_ij];
        end
    ...
    % add axis labels and title
    title_label = join(["Enh-Cas9 HNH Residence Times, MM=",num2str(num_mismatches(ii))]);
